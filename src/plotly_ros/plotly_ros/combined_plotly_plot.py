...
-------------------------------------------------------------------
This script shows you **how** to extend a basic odometry plot into a full
multi-sensor dashboard.  Study the extra  **COMMENTS** - they explain the
steps you will repeat whenever you add another topic.

Visualised streams  ⇢  Tabs created in Dash
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
def launch_dash(node: CarlaDataBuffer):
    ...
    app.layout = html.Div([
        ...
        dcc.Tabs([
            # Tab 1 – Odometry
            dcc.Tab(label='Odometry (x, y)', children=[
                dcc.Graph(id='odom-plot'),
            ]),
            # Tab 2 – GNSS
            dcc.Tab(label='GNSS (Lat, Lon)', children=[
                dcc.Graph(id='gnss-plot'),
            ]),
            # Tab 3 – IMU
            dcc.Tab(label='IMU Accel', children=[
                dcc.Graph(id='imu-plot'),
            ]),
            # Tab 4 – Speedometer
            dcc.Tab(label='Speed (m/s)', children=[
                dcc.Graph(id='speed-plot'),
            ]),
            # Tab 5 – Camera
            dcc.Tab(label='Front Camera', children=[
    ...
    # ---------- start Dash --------------------------------------------------
    # Dash ≥3 recommends app.run(); run_server() still works but emits a warning.
